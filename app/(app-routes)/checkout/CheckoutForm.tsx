...
const CheckoutFormSchema = z.object({
  ...
  area: z
    .string()
    .min(5, {
      message: 'Area must be at least 5 characters.',
    })
    .max(30, {
      message: 'Area must not be longer than 30 characters.',
    }),
  phoneNumber: z
    .string()
    .min(7, {
  ...
  provinceId: z.string({
    required_error: 'Please select a province to display.',
  }),
  cityId: z.string({
  ...
  landmark: z
    .string()
    .max(50, {
      message: 'Landmark must not be longer than 50 characters.',
    })
    .min(4, {
      message: 'Landmark must be longer than 4 characters.',
    }),
  //   urls: z
  //     .array(
  //       z.object({
...
export default function CheckoutForm() {
  ...
  const getAddress = async () => {
    ...
    try {
      ...
      setDeliveryData({ id: data.data[0].deliveryChargeId, amount: data.data[0].deliveryCharge });
    } catch (error) {
      // Handle the error
    } finally {
      setLoading(false);
  ...
  const deleteAddress = async () => {
    const formData = {
      id: addresss && addresss?.id,
    };
    setLoading(true);
    try {
      const res = await fetch(`/api/cart/remove-address`, {
        method: 'POST',
    ...
    return (
      <>
        <div className="flex items-start justify-start mb-5">
          <div className="rounded border bg-card text-card-foreground shadow">
            <div className="flex flex-col space-y-1.5 p-6">
              <h3 className="font-semibold leading-none tracking-tight">{`${addresss.firstName} ${addresss.lastName}`}</h3>
              <p className="text-sm text-muted-foreground">{`${addresss.area}, ${addresss.nearestLandmark}`}</p>
              <p className="text-sm text-muted-foreground">{`${addresss.cityName}, ${addresss.provinceName}`}</p>
            </div>
            <div className="flex items-center px-4 pt-0 border-t">
              <p className="text-sm py-4 text-muted-foreground">You package will be delivered to this address</p>
            </div>
          </div>
        </div>
        <span className="text-grey ">
          You want to ship to another address?{' '}
          <span className="text-[#800000] cursor-pointer" onClick={() => deleteAddress()}>
            {' '}
