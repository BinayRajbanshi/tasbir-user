...
export default function RootLayout() {
  ...
  const [submitEsewaForm, setSubmitForm] = useState(false);
  ...
  const createOrder = async () => {
    ...
    try {
      ...
      console.log(data.message == 'Order has been placed successfully.');
      if (data.message == 'Order has been placed successfully.') {
  ...
  const getPaymentData = async () => {
    ...
    try {
      ...
      if (data) {
        setLoading(false);
        console.log('data esewa', data.data);
        setEsewaData(data.data);
      }
      console.log(data);
    } catch (error) {
  ...
  useEffect(() => {
    // When esewaData becomes available, automatically submit the form
    if (esewaData && !formSubmitted) {
      submitForm();
      setFormSubmitted(true);
    }
  }, [esewaData, formSubmitted]);

  const handleSubmit = (e) => {
    e.preventDefault();

    // Add any additional checks if needed before submitting the form
    // if (!formSubmitted) {
    //   setFormSubmitted(true);
    //   e.target.submit();
    // }
  ...
  return (
    <div className="flex flex-col justify-start items-start">
      <form ref={formRef} className="w-full" action="https://rc-epay.esewa.com.np/api/epay/main/v2/form" method="POST">
        ...
        <input
          ...
          defaultValue={esewaData?.product_delivery_charge}
          required
        />
